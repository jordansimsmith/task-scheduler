...
public class IterativeDeepeningAStarSchedule implements ISchedule, Comparable<IterativeDeepeningAStarSchedule> {
    ...
    private Set<IterativeDeepeningAStarSchedule> childStates;

    private int scheduledNodeCount;

    private IterativeDeepeningAStarSchedule() {
    }
     ...
     */
    public IterativeDeepeningAStarSchedule(List<INode> free, Map<INode, Integer> parentCounter) {
        this.free = free;
     ...
     */
    public IterativeDeepeningAStarSchedule expand(INode node, int processor) {
        IterativeDeepeningAStarSchedule s = new IterativeDeepeningAStarSchedule();

        int lastNodeStartTime = minStartTime(node, processor);
        s.maxBottomLevelCost = Math.max(this.maxBottomLevelCost, lastNodeStartTime + IterativeDeepeningAStar.bottomLevelCache.get(node));
        s.idleTime = this.idleTime + lastNodeStartTime - this.earliestTimes[processor - 1];
        s.idleTimeHeuristicValue = (s.idleTime + IterativeDeepeningAStar.totalNodeWeighting) / Config.getInstance().getNumberOfCores();
     ...
     * The schedule field allows for a more efficient equality check of IterativeDeepeningAStarSchedule and
     * thus faster duplicate detection.
     */
    private void populateScheduleString() {
        StringJoiner joiner = new StringJoiner(" ");
        for (INode node : IterativeDeepeningAStar.sortedNodes) {
    ...
    @Override
    public int compareTo(IterativeDeepeningAStarSchedule o) {
    ...
    public boolean equals(Object o) {
        if (o instanceof IterativeDeepeningAStarSchedule) {
            return this.scheduleString.equals(((IterativeDeepeningAStarSchedule) o).scheduleString);
        }
        return false;
    }
    ...
    public Set<IterativeDeepeningAStarSchedule> getChildStates() {
        if(childStates == null) {
