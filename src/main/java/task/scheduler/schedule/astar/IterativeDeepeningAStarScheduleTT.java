...
public class IterativeDeepeningAStarScheduleTT implements ISchedule, Comparable<IterativeDeepeningAStarScheduleTT> {
    ...
    private Set<IterativeDeepeningAStarScheduleTT> childStates;

    private int scheduledNodeCount;

    private IterativeDeepeningAStarScheduleTT() {
    }
     ...
     */
    public IterativeDeepeningAStarScheduleTT(List<INode> free, Map<INode, Integer> parentCounter) {
        this.free = free;
     ...
     */
    public IterativeDeepeningAStarScheduleTT expand(INode node, int processor) {
        IterativeDeepeningAStarScheduleTT s = new IterativeDeepeningAStarScheduleTT();

        int lastNodeStartTime = minStartTime(node, processor);
        s.maxBottomLevelCost = Math.max(this.maxBottomLevelCost, lastNodeStartTime + IterativeDeepeningAStarTT.bottomLevelCache.get(node));
        s.idleTime = this.idleTime + lastNodeStartTime - this.earliestTimes[processor - 1];
        s.idleTimeHeuristicValue = (s.idleTime + IterativeDeepeningAStarTT.totalNodeWeighting) / Config.getInstance().getNumberOfCores();
     ...
     * The schedule field allows for a more efficient equality check of IterativeDeepeningAStarScheduleTT and
     * thus faster duplicate detection.
     */
    private void populateScheduleString() {
        StringJoiner joiner = new StringJoiner(" ");
        for (INode node : IterativeDeepeningAStarTT.sortedNodes) {
    ...
    @Override
    public int compareTo(IterativeDeepeningAStarScheduleTT o) {
    ...
    public boolean equals(Object o) {
        if (o instanceof IterativeDeepeningAStarScheduleTT) {
            return this.scheduleString.equals(((IterativeDeepeningAStarScheduleTT) o).scheduleString);
        }
        return false;
    }
    ...
    public Set<IterativeDeepeningAStarScheduleTT> getChildStates() {
        if(childStates == null) {
